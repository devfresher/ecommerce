...
import { FindAllOption, RelationOptions } from '../typings/core';
import { Injectable, NotFoundException } from '@nestjs/common';
import { ClientSession, FilterQuery, HydratedDocument, Model, PopulateOptions } from 'mongoose';
import { PaginatedResult, PaginationMetadata } from '../typings/paginate';

@Injectable()
export abstract class BaseService<T, D extends HydratedDocument<T>> {
  /**
   ...
   */
  async countAll(filter?: FilterQuery<T>, session?: ClientSession): Promise<number> {
   ...
   */
  async getOrError(opts?: FindAllOption<T>, session?: ClientSession): Promise<D> {
   ...
   */
  async get(opts?: FindAllOption<T>, session?: ClientSession): Promise<D | null> {
